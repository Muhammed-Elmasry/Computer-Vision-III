...
class DavisDataset(Dataset):
    def __init__(
        ...
        path: Path,
        patch_size: int,
        image_names: Optional[List[Tuple[str, str]]] = None,
    ) -> None:
        ...
        # init if image names already provided
        if image_names is not None:
            self.image_names = image_names
            for _, category_name in self.image_names:
        ...
        for category_dir in sorted(self.dataset_path.iterdir()):
            ...
            for image in sorted(category_dir.joinpath(self.image_dir).iterdir()):
                ...
                name = image.stem
                if (
                    category_dir.joinpath(self.feature_maps_dir)
                    .joinpath(f"{name}.pt")
                    .is_file()
                    and category_dir.joinpath(self.annotation_dir)
                    .joinpath(f"{name}.png")
                    .is_file()
                    and category_dir.joinpath(self.annotation_coarse_dir)
                    .joinpath(f"{name}.png")
                    .is_file()
                ):
    ...
    def __getitem__(self, index) -> Dict[str, Any]:
        ...
        img = load_img(
            self.dataset_path.joinpath(category_name, self.image_dir, f"{name}.jpg")
        )
        feature_map = load_feature_map(
            self.dataset_path.joinpath(
                category_name, self.feature_maps_dir, f"{name}.pt"
            )
        )
        annotations = load_annotation(
            self.dataset_path.joinpath(
                category_name, self.annotation_dir, f"{name}.png"
            )
        )
        annotations_coarse = load_annotation(
            self.dataset_path.joinpath(
                category_name, self.annotation_coarse_dir, f"{name}.png"
            )
        )
    ...
    def split_categories(self) -> Dict[str, Self]:
        datasets: Dict[str, Self] = {}
        for category in self.category_names:
            image_names = [
                (name, category_name)
                for name, category_name in self.image_names
                if category_name == category
            ]
            datasets[category] = DavisDataset(
                self.dataset_path, self.patch_size, image_names
            )
...
def create_embeddings(path: Path, dino_model, device: torch.device) -> None:
    ...
    dino_model._modules["blocks"][-1]._modules["attn"]._modules[
        "qkv"
    ...
    for category_dir in sorted(path.iterdir()):
        ...
        for image_path in tqdm(image_paths):
            ...
            image = image[
                ...,
                : math.floor(image.shape[-2] / dino_model.patch_embed.patch_size)
                * dino_model.patch_embed.patch_size,
                : math.floor(image.shape[-1] / dino_model.patch_embed.patch_size)
                * dino_model.patch_embed.patch_size,
            ]
            ...
            torch.save(
                keys.reshape(H_patch, W_patch, -1),
                category_dir.joinpath(feature_map_dir, f"{image_path.stem}.pt"),
            )
...
def downsample_annotations(path: Path, patch_size: int) -> None:
    ...
    for category_dir in sorted(path.iterdir()):
        if not (
            category_dir.is_dir() and category_dir.joinpath(annotations_dir).is_dir()
        ):
            continue
        ...
        annotation_paths = [
            ...
            for annotation_path in sorted(
                category_dir.joinpath(annotations_dir).iterdir()
            )
        ...
        for annotation_path in tqdm(annotation_paths):
            ...
            save_annotation(
                category_dir.joinpath(
                    annotations_coarse_dir, annotation_path.parts[-1]
                ),
                annotation_coarse,
            )
...
def load_davis_dataset(
    path: Path, patch_size: int
) -> Tuple[DavisDataset, DavisDataset]:
    return DavisDataset(path.joinpath("train"), patch_size), DavisDataset(
        path.joinpath("test"), patch_size
    )
...
def visualize_davis(dataset: DavisDataset, index: int) -> None:
    ...
    pca = PCA(3, True, data["feature_map"].flatten(0, 1))
    pca_map = pca.do_pca(data["feature_map"].flatten(0, 1))
    print(pca_map.mean(dim=0))
    print(pca_map.max(dim=0)[0])
    print(pca_map.min(dim=0)[0])
    pca_min = torch.min(pca_map, dim=0, keepdim=True)[0]
    pca_max = torch.max(pca_map, dim=0, keepdim=True)[0]
    pca_map = ((pca_map - pca_min) / (pca_max - pca_min)).reshape(H_feat, W_feat, 3)
    ...
    axs[3].imshow(
        upsample(pca_map.permute(2, 0, 1)[None])[0]
        .permute(1, 2, 0)
        .mul(255)
        .byte()
        .cpu()
